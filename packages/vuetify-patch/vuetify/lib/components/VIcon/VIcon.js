...
var VIcon = mixins(Colorable, Sizeable, Themeable
  ...
  render: function render(h) {
    ...
    svgData.attrs.width = parseFloat(fontSize || 24);
    svgData.attrs.height = svgData.attrs.width;
    svgData.attrs.viewBox = '0 0 24 24';
    svgData.attrs.version = '1.1';
    svgData.attrs.xmlns = 'http://www.w3.org/2000/svg';

    // var $icons = typeof window !== 'undefined' ? window.$icons : global.$icons;
    var $icons = this.$vuetify.options.svgIcons

    iconName = iconName ? iconName.trim() : iconName

    if (iconName && (!$icons || !$icons[iconName])) {
      var $missingIcons = typeof window !== 'undefined' ? window.$missingIcons : global.$missingIcons;
    ...
    var pathData = ($icons && $icons[iconName]) || (iconName ? 'M4 4 H 20 V 20 H 4 L 4 4' : ' ');

    svgData.domProps = {
      innerHTML: pathData[0] === '<' ? '<g>' + pathData + '</g>' : '<path d="' + pathData + '" />'
    };

    return h('i', this.setTextColor(this.color, data), [h('svg', svgData)]);
  }
});
