...
export default {
  ...
  computed: {
    grouped (): IGrouped[] {
      ...
      const mappedRecords: IGrouped[] = map(groupBy(data, this.grouping.select), (records: any[], title: string) => ({
        prop: this.propName(title),
        records,
        title
      }))
      return <IGrouped[]> orderBy(mappedRecords, [(d: any) => d.records.length, TITLE], [DECS, ASC])
    },
    top (): IGrouped[] {
      if (!this.grouped) {
        return null
