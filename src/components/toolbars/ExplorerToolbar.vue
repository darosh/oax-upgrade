<template lang="pug">
  v-layout.ma-0.v-toolbar__content.px-0(row style="height: 100%")
    ...
    template(v-if="SPEC")
      ...
      div(class="searching", :class="{'searching--closed': !searching}")
        v-text-field(spellcheck="false" id="search" append-icon="close", @click:append="searchEnd" v-model="search", name="search", label="Search", single-line)
      v-tooltip(bottom)
