<template lang="pug">
  div
    ...
    v-layout(style="z-index: 9").pt-3.pb-3.pl-3.pr-3.ma-0.elevation-2.relative
      v-text-field.mr-2(spellcheck="false" solo label="Search" v-model="filter" hide-details single-line prepend-icon="search", :append-icon="filter ? 'close' : null", @click:append="() => filter = null" v-focus.wait="UI_LEFT_DRAWER && value")
      v-menu(bottom left)
    ...
    v-tabs-items(:value="tab")
      ...
      v-tab-item.scroller(value="tab-dir-2", v-if="APIS_COLLECTION_OBJECT.categories")
...
import * as directory from '../../../scripts/services/directory'
...
export default {
  ...
  computed: {
    ...
    uncategorized () {
      return this.APIS ? this.APIS.filter(d => !d.categories).length : 0
    },
    collection: {
      get () {
    ...
    filtered () {
      if (!this.category &&
        ...
        // TODO no-side-effects-in-computed-properties
